describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const buttonText = await page.$eval('product-item', prodItems => {
      const shadowRoot = prodItems.shadowRoot;
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allRemoveButtons = await page.$$eval('product-item', items => {
      return items.every(item => {
        return item.shadowRoot.querySelector('button').innerText === 'Remove from Cart';
      })
    });

    const cartCount = await page.$eval('#cart-count', el => parseInt(el.innerHTML));
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for (let item of prodItems) {
      const shadowRoot = await item.evaluateHandle(el => el.shadowRoot);
      const button = await shadowRoot.$('button');
      if ((await button.evaluate((btn) => btn.innerText)) === 'Remove from Cart') {
        await button.click();
      }
    }

    const cartCount = await page.$eval('#cart-count', (cartCount) => {
      return parseInt(cartCount.innerText);
    });
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allAddButtons = await page.$$eval('product-item', items => {
      return items.every(item => {
